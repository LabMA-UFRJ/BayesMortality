...
#'
#' @description This function mixes the fitted mortality table of a HP model with another mortality
...
#' @param mu_post Vector with mortality rates used in the mix.
#' @param weights Positive vector specifying the weights used in the mix. The default value is NULL.
#' @param mix_age Positive vector specifying the age range in the mixture.
#' @param x0_prior Nonnegative number indicating the initial age of the fitted HP model.
#' @param x0_post Nonnegative number indicating the initial age of the mortality table provided by the user.
#' @param max_age Positive number indicating the final age in the mixture.
#'
#' @return Return new chains for qx.
#'
#' @examples
...
hp_mix = function(fit, mu_post, weights = NULL, mix_age, x0_prior, x0_post, max_age) {
  ...
  if(class(fit) != "HP") stop("fit must be an object of the class HP.")
