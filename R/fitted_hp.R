...
#'
#' @description This function calculates the point estimations of the death probabilities (qx) of a 'HP' or 'ClosedHP' class object adjusted by the hp() or hp_close() functions.
#'
#' @usage
#' fitted(fit, age = NULL)
#'
#' @param fit Object of the class 'HP' or 'ClosedHP' adjusted by the hp() or hp_close() functions.
#' @param age Vector with the ages to calculate the death probabilities (Optional).
#'
...
#' fit = hp(x = x, Ex = Ex, Dx = Dx, M = 40000, thin = 30)
#'
...
#'
#' @export
fitted.HP <- function(fit, age = NULL){
  if(is.null(age)){
...
#' @export
fitted.ClosedHP <- function(fit, age = NULL){
