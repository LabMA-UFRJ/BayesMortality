...
#' @param model Character string specifying the model to be used. The options are: "binomial", "lognormal" or "poisson".
#' @param M Positive integer indicating the number of iterations of the MCMC run.
#' @param bn Nonnegative integer indicating the number of iteration to be discarded as the burn-in period.
...
#' @param K Number that specifies the extra paramater 'K' value for binomial and poisson models. It is used only if model = "binomial" or model = "poisson".
#' @param sigma2 Positive number that specifies initial value of sigma2 in lognormal model. It is used only if model = "lognormal".
#' @param prop.control Positive number that is used for tuning the acceptance rate of MCMC.
#' @param reduced_model Logical value which determines if reduced model should be used.
#'
#' @details
...
hp <- function(x, Ex, Dx, model = c("binomial", "lognormal", "poisson"), M = 50000, bn = round(M/2),
  ...
  if(any(m[c(1:5,7,8)] <= 0) || any(m[c(1:4,7)] >= 1) || m[6] <= 15 || m[6] >= 110){param = LETTERS[1:8]; stop(sprintf("A média do(s) parâmetro(s) %s não está no intervalo correto.", paste(param[which((m <= lower || m >= upper))], collapse = ", ")))}
